...
public class ParticipationController {
    ...
    @Parameters(value = {
            @Parameter(name = "challengeId", description = "챌린지(게시글) ID"),
            @Parameter(name = "pageable", description = "size 는 디폴트로 5를 지정해놓았기 때문에 page 만 전달해주면 됨")
    })
    @GetMapping("/view/{challengeId}")
    public ApiResponseDTO<?> viewAllParticipation(@PathVariable Long challengeId, @PageableDefault(size = 5) Pageable pageable) {
        // 숨김 처리한 참여 게시글 조회를 위해 로그인한 사용자의 ID 를 추출
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        Member member = (Member) authentication.getPrincipal();
        DataWithCount<?> dataWithCount = participationService.findParticipation(member.getMemberId(), challengeId, pageable);
    ...
    public ApiResponseDTO<?> uploadImage(ImageSaveDTO imageDTO) {
        ...
        try {
            ...
            return new ApiResponseDTO<>(BAD_REQUEST, FAIL, "게시글에 존재하지 않는 지점입니다", null);
        }
